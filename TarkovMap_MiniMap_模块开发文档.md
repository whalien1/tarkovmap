...
> 当前实现同步：TarkovMap v1.1.2（2026-08-30 冻结）

---

## 0. 需求确认记录（2026-08-24 Q&A，10/10 已锁定）

以下为开发前与用户对齐的决策记录。若与本文“完成记录”、README 或 AI 交接维护手册描述的已发布行为冲突，以已发布行为为准。
...
4. **默认外观**：方形（Square）+ 透明度中（75%）+ 中（300px；小 260px、大 340px @96DPI）+ 主屏右上角。
5. **M0 共享状态重构**：接受。完成后主程序行为必须与 v1.0 完全一致，按 v1.0 全清单回归验收。
6. **显示器适配**：只适配单显示器；保留基本屏幕边缘保护，不做多屏恢复逻辑。
7. **鼠标焦点**：不做鼠标穿透（维持正文 §29），小地图仍可拖动和滚轮缩放；已通过 `MA_NOACTIVATE` 保证鼠标操作不抢走游戏焦点。
8. **开发阶段划分**（替代正文 §39 的 M0~M7 呈现给用户）：
   - M0 内部翻新（MapViewState 重构，无新功能，全清单回归）
   - M1 最小可用小地图（方形 300px、75% 透明度、右上角；玩家居中、箭头朝向、滚轮缩放）
   - M2 图标同步（按第 3 条规则固定显示四类内容）
   - M3 窗口交互+外观（拖动、位置记忆、透明度三档、大小三档、圆/方切换、主界面设置区、等待定位提示）
   - M4 生命周期+性能（最小化/失焦仍显示、随主程序退出、内存不翻倍、静止零持续重绘）
9. **v1.0 遗留项**：立交桥"河畔之路（信号弹）"撤离点坐标仍为估算值，本次迭代不处理，待办保留。
10. **收尾方式**：M4 验收通过后同步更新主文档+本文档、打 Git 标签、打包正式客户端。当前已随 `TarkovMap-v1.1.2.zip` 发布并冻结。
...
配置只保存 `small / medium / large`，不保存任意宽高。
...
窗口通过 `WM_MOUSEACTIVATE → MA_NOACTIVATE` 保持鼠标消息可用，同时不激活窗口；因此拖动和滚轮缩放不会打断游戏输入。
...
## 47. 完成记录（v1.1.2，2026-08-30 已冻结）

- **M0–M4 全部完成并通过用户实测验收**：共享状态重构、最小可用小地图、图标同步、窗口交互+外观、生命周期+性能。
- **实测性能**：开小地图内存增量约 0.2MB（67.0 vs 66.8MB），共享 Bitmap 设计达标。
- **实测确认的默认设置**：方形 / 中（300px；小 260px、大 340px）/ 透明度中 / 右上角；设置项用下拉框（高 DPI 下单选按钮会重叠）。
- **焦点行为**：窗口使用 `MA_NOACTIVATE`，可拖动、可滚轮缩放，但鼠标操作不抢游戏焦点。
- **朝向定稿**：箭头角度 = Yaw + coordinateRotation + 90°；地图级校准以 Builder 的 `calibration-v1.1.1.json` 为准。
