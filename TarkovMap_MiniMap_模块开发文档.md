...
3. **Marker 显示规则（修订正文 §3 的"完全同步"）**：小地图**固定只显示** 撤离点 + Boss + 危险区 + 地区名标注，与主地图勾选状态**解耦**；第一版规则定死，后续迭代再开放。
4. **默认外观**：方形（Square）+ 透明度中（75%）+ 大（360px @96DPI）+ 主屏右上角。
...
8. **开发阶段划分**（替代正文 §39 的 M0~M7 呈现给用户）：
   ...
   - M1 最小可用小地图（方形 360px 不透明右上角；玩家居中、箭头朝向、滚轮缩放）
...
但最终像素尺寸必须在 1080p / 2K / 4K 与不同 DPI 下实测后锁定。
